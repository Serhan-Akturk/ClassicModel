...
-- Day2 SQL questions
-- 1) Use in built MySql functions to display how many years ago a classicmodels order was shipped (shipped date) as "Years Ago". Only count orders that are "shipped" status and 
-- order by Years Ago oldest to newest. Include order number and customer number. Hint: look at Age example
...
-- 2) Use in built MySql functions to query from the classicmodels payments table with four columns. One being unchanged amount column, amount rounded up, amount rounded down, and 
-- amount in pesos. Use 1 dollar = 20.63 pesos, make sure this is formatted to two decimals. Include aliases 
...
-- 3) Create a custom function to display profit return ratings (returnRating(MSRP,buyPrice)) for classic models products using MSRP minus buyPrice for profit margin.  
...
-- 4) Create two views, one as ratings_view and the other as productline_view ratings_view should have three columns, the return rating from the function created previously, number of 
-- products (count), and average profit margin(avg of msrp-buyprice) with corresponding aliases this should be grouped by the return rating function and ordered by the average profit 
-- margin in descending order. productline_view should be similar to ratings view, except you will replace the return rating column with productline abd group by productline. 
...
-- 5) Create a simple procedure that will return a table that returns a left join of employees and offices. This table should only have employees whose job title is sales rep and be 
-- ordered by officecode the procedure can be called GetEmpOfficeRep() 
...
-- 6) Create a procedure with cursor to create a list of the different office cities in offices table with commas between. Exclude the remote office with city as remote. 
